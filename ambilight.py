...
GPIO.setup(21, GPIO.IN, pull_up_down=GPIO.PUD_UP)# Our function on what to do when the button is pressed
...
for frame in camera.capture_continuous(rawCapture, format="rgb", use_video_port=True):
	if SHUTDOWN:
		for x in range(0,LED_COUNT):
			strip.setPixelColor(x, toRGB(0,0,0))
			strip.show()
			time.sleep(0.1)
	...
	inp=frame.array
	inp=np.where(inp<20,0,inp)
	image = np.add(np.multiply(np.divide(image,INTERPOLATION_SMOOTHNESS),INTERPOLATION_SMOOTHNESS-1),np.divide(inp,INTERPOLATION_SMOOTHNESS))
	...
	for x in range(0,59):
		...
		strip.setPixelColor(59*2+33*2+1-x, toRGB(r,g,b))
	for x in range(0,34):
	...
	for x in range(0,34):
		...
		strip.setPixelColor(34+59+x, toRGB(r,g,b))
